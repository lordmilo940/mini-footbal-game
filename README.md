...
function update() {
    ...
    if (ai.reactionDelay <= 0) {
        let errorFactor = (Math.random() - 0.5) * 50;
    ...
    // --- CORRECCIÓN DEFINITIVA DE ESQUINAS Y BORDES ---
    // Rebotes Techo y Piso básicos
    ...
    // Colisión de pared Izquierda (Jugador)
    if (ball.x - ball.radius < 0) {
        // ¿Está dentro del rango de altura del arco?
        if (ball.y > goals.left.y && ball.y < goals.left.y + goals.left.height) {
            // Entró al arco completamente
            if (ball.x + ball.radius < 0) {
                aiScore++;
            ...
            // Chocó contra la pared exterior (Esquina superior o inferior izquierda)
            ball.x = ball.radius;
            ball.vx = -ball.vx * 0.8;
            // Si además se arrastra trabada en el rincón exacto, la despega hacia el centro
            if (ball.y < 20) ball.vy = 2;
            if (ball.y > canvas.height - 20) ball.vy = -2;
        }
    }

    // Colisión de pared Derecha (IA)
    if (ball.x + ball.radius > canvas.width) {
        ...
        if (ball.y > goals.right.y && ball.y < goals.right.y + goals.right.height) {
            ...
            // Chocó contra la pared exterior (Esquina superior o inferior derecha)
            ball.x = canvas.width - ball.radius;
            ball.vx = -ball.vx * 0.8;
            // Despegue de seguridad si se desliza muerta en el rincón
            if (ball.y < 20) ball.vy = 2;
            if (ball.y > canvas.height - 20) ball.vy = -2;
        }
    }
